...
def a_star_search(grid):
    start = next((x, y) for x, row in enumerate(grid) for y, val in enumerate(row) if val == 2)
    goal = next((x, y) for x, row in enumerate(grid) for y, val in enumerate(row) if val == 3)
    open_set = []
